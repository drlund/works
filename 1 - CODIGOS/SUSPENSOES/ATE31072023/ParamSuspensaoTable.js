...
function ParamAlcadasTable({ ...props }) {
  ...
  useEffect(() => {
    if (
      ...
      setFetching(true);
      Promise.all([obterParametros(), getParametros(idParametro)])
  ...
  const getPrefixoDestino = (/** @type {{ prefixoDestino: string; }} */ parametro) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (/** @type {string} */ prefixoDestino) => String(prefixoDestino).padStart(4, '0'),
    },
    {
      title: 'Nome Prefixo',
      ...
      return getParametros(idParametro)
        .then((onRow) => {
          const parametrosFiltrados = onRow.filter(
            (/** @type {{ prefixoDestino: import('lodash').String; }} */ parametro) => parametro.prefixoDestino === prefixo,
          );
  ...
  const removerParametro = (/** @type {number} */ id) => {
