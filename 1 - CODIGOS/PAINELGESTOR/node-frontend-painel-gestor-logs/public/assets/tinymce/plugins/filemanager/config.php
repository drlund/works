...
$site_uri = substr($_SERVER['REQUEST_URI'], 1, strpos(substr($_SERVER['REQUEST_URI'], 1), '/'));
$root = $root . "/$site_uri/";
...
$url .= '://' . $_SERVER['HTTP_HOST'] . '/' . $site_uri;
//**********************
//Path configuration
//**********************
...
$base_url=$url; //url base of site if you want only relative url leave empty
$upload_dir = isset($_SESSION['upload_folder']) ? $_SESSION['upload_folder'] : '/uploads/disp_mail/'; // path from base_url to upload base dir
$current_path = '../../../../../uploads/disp_mail/'; // relative path from filemanager folder to upload files folder
...
$ext_file = array('doc', 'docx', 'pdf', 'xls', 'xlsx', 'xlsm', 'txt', 'csv','html','psd','sql','log','fla','xml','ade','adp','ppt','pptx', 'ods', 'odt'); //Files
$ext_video = array('mov', 'mpeg', 'mp4', 'avi', 'mpg','wma', 'webm'); //Videos
