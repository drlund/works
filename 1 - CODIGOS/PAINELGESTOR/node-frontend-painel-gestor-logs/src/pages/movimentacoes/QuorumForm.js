...
function QuorumForm() {
  ...
  const onSalvarMeuPrefixo = async (atualQuorum, observacao) => {
    if (prefixoNaLista) {
      try {
        await patchQuorumMeuPrefixo(atualQuorum, observacao);
      } catch (error) {
      ...
      try {
        await postQuorumMeuPrefixo(atualQuorum, observacao);
      } catch (error) {
        message.error('Erro ao salvar prefixo próprio.');
      } finally {
        onRecarregar();
          ...
          <Col span={24}>
            <Card title="Meu Prefixo">
              <QuorumCard
                ...
                onUpdateObservacao={setFormObservacao}
                observacao={formObservacao}
              />
            </Card>
          </Col>
