...
const idSubsidiariaTable = {
  1: subsidiarias.BB,
  2: subsidiarias.Cartoes,
  3: subsidiarias.Consorcio,
};

export function getCustoManifesto(/** @type {number|null} */ idSubsidiaria) {
  const custoBB = 18.54;
  ...
  return {
    ...
    // @ts-expect-error possible undefined as index
...
export function extractIdSubsidiaria(/** @type {Procuracoes.Procuracao} */ procuracaoOutorgado, /** @type {number} */ procuracaoId) {
  ...
  return procuracaoOutorgado.subsidiarias.find((s) => s.procuracaoId === procuracaoId)?.id ?? null;
}
