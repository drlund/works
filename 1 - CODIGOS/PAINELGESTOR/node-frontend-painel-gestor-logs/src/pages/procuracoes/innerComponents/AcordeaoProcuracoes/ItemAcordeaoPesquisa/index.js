 ...
 * } & import('antd').CollapsePanelProps} props
 */
export function ItemAcordeao({ isShowing, outorgado, ...props }) {
  ...
  const ItemWrapper = useCallback(
    ...
    ({ children }) => {
      const isRevogado = (() => {
        try {
          return Boolean(dataSource[0].revogacao);
        } catch {
          return false;
        }
      })();

      const makeExtraHeader = () => {
        if (isRevogado) {
    ...
    return (
      <ItemWrapper>
        <Table
          ...
          // @ts-expect-error tipo das colunas
          dataSource={outorgado.procuracao[0].subsidiarias}
          pagination={false}
          rowKey={(record) => `${record.procuracaoId}`}
          onRow={(record) => ({
            style: {
              backgroundColor: getRevogadoInativoColor({ revogado: record.revogacao, ativo: record.procuracaoAtiva }),
            }
          })}
        />
      </ItemWrapper>
