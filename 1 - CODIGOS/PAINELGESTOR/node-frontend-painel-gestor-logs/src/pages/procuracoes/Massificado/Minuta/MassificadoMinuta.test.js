...
it('<MassificadoMinuta>', async () => {
  ...
  // render the proximo button disabled
  expect(screen.getByRole('button', { name: /próximo/i })).toBeDisabled();

  // on adding a lista de matriculas
  // @ts-ignore
  ...
  // render funcis in correct order
  ...
  // has the proximo button enabled
  ...
  // in the finalizar etapa
  // renders the outorgante section
  ...
  // renders the outorgados & minutas section
  expect(screen.getByRole('heading', {
  ...
  // creates a different id for each of the massificado and outorgados
