...
import { authStateChange, updateUserPermissions } from '../../services/actions/commons';
...
export const getSessionState = () => (window.bbapi ? window.bbapi.getState() : {});

export const isLoggedIn = () => (window.bbapi ? window.bbapi.isLoggedIn() : false);

export const verifyUnauthorizedStatus = (response) => {
  if (response && response.status === 401 && response.data) {
    if (response.data.errorType && response.data.errorType === 'invalid_token') {
      if (window.bbapi) {
...
class Authentication extends React.Component {
  ...
  getPermissoesUsuario = (token, fromAction) => {
    ...
    apiService.get('/acessos/permissoes')
      .then((response) => {
      ...
      .catch((error) => {
        ...
        if (fromAction) {
          message.error('Falha ao recarregar os seus acessos. Tente novamente.');
        }
      });
  };

  render() {
    const atualToken = (this.props.atualToken && this.props.atualToken.length)
    ...
    if (this.props.isLoggedIn === false && atualToken) {
      return (
        <ModalAuth visible token={atualToken} />
      );
    }
    return null;
  }
