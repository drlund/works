...
class FramePage extends Component {
  ...
  renderPageContent = ({
    ...
    const { authState, configureTrackerId } = this.props;
  ...
  renderAllRoutes = () => {
    const routeItems = Object.keys(menuConfig).map((menuKey) => menuConfig[menuKey]);

    return routeItems.map((item) => item.routes.map((route) => (
      <Route
        path={route.path}
        exact
        render={(props) => this.renderPageContent({
          routeCfg: route,
          props,
          name: item.name,
...
const mapStateToProps = (state) => ({ authState: state.app.authState });
