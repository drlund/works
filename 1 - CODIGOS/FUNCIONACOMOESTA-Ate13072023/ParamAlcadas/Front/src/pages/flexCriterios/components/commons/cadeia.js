...
export default function Cadeia({ acao, pedidoFlex, perfil }) {
  ...
  console.log(pedidoFlex.manifestacoes);

  const consultarManifestacao = (manifestacao) => {
    setConteudoModal(manifestacao);
    setModalAberta(true);
    // pode ser que peçam que mude de nova aba para modal
    // Modal.confirm({
    ...
    //         {manifestacao?.matricula} - {manifestacao?.nome}
    //       </Typography.Text>
    //       <Typography.Text>
    //         <Typography.Text strong>{manifestacao?.acao?.nome}</Typography.Text>:{' '}
    //         {manifestacao?.texto}
    //       </Typography.Text>
    //         <Typography.Text disabled>{manifestacao?.acao?.nome} registrada em {manifestacao?.updatedAt}</Typography.Text>
    //     </Space>
    //   ),
    //   cancelText: 'Voltar',
    //   onCancel() {},
    //   okText: 'Solicitar Complemento',
    //   onOk() {},
    // });
  };
      ...
      >
        <Space direction="vertical">
          ...
          <Typography.Text disabled>
            ...
            {/*  {conteudoModal?.acao?.nome} registrada em {conteudoModal?.updatedAt} */}
          </Typography.Text>
        </Space>
      </Modal>
    </CardResponsivo>
