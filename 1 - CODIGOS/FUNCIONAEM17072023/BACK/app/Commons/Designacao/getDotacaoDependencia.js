...
async function getDotacaoDependencia(dependencia, ger, gest) {
  ...
  try {
    ...
    qtdeFuncisDotados = dotacao.map(elem => elem.qtde_dotacao).reduce(((acc, cur) => acc + cur), 0);
    qtdeFuncisLotados = dotacao.map(elem => elem.qtde_lotacao).reduce(((acc, cur) => acc + cur), 0);
    qtdeFuncisExistentes = dotacao.map(elem => elem.qtde_existencia).reduce(((acc, cur) => acc + cur), 0);
    ...
    if (ger || gest) {
      ...
      terGUN = dotacao.filter(elem => !_.isEmpty(_.intersection(ref_org_3gun, [elem.cod_cargo]))).map(elem => elem.qtde_existencia).reduce(((acc, cur) => acc + cur), 0);
    ...
    return { dotacao, terGUN, qtdeFuncis: qtdeFuncisDotados };
  }catch (err) {
