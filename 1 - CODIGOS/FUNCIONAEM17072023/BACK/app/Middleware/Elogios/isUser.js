...
const PERMISSOES_REQUERIDAS = ['REGISTRAR_ELOGIO'];
...
class IsUser {
  ...
  async handle ({ session }, next) {
    ...
    if(!possuiAcesso){
      throw new exception(`IsUser: Funcionário ${session.get('currentUserAccount').chave} não possui a(s) permissão(ões) ${PERMISSOES_REQUERIDAS.join(',')}`, 400, 'MISSING_PERMISSION');
    }
