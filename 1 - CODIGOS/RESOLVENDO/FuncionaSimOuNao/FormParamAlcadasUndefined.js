...
function FormParamAlcadas({ location }) {
  ...
  const [jurisdicaoUsuario, setJurisdicaoUsuario] = useState('');
  const [prefixoSubordinada, setPrefixoSubordinada] = useState();
  ...
  const { prefixo, matricula, nome_usuario: nomeUsuario } = dadosDoUsuario;
    ...
    if (cod_dependencia) {
      getCargosComissoesFot09(cod_dependencia)
        .then((result) => {
          ...
          setPrefixoSubordinada(cod_dependencia);
        })
        .catch(() => {
  ...
  function gravaParametros() {
    form
      ...
      .then((dadosForm) => {
        ...
        const { prefixoDestino, cod_dependencia } = dadosForm;
  
        if (prefixoDestinoValue && prefixoDestinoValue !== prefixo || prefixoDestinoValue !== cod_dependencia ) {
