...
function ParamAlcadasTable({ ...props }) {
  ...
  const obterParametros = () => {
    if (permissao.includes('PARAM_ALCADAS_ADMIN')) {
      ...
      const prefixoDestinoUsuario = getJurisdicoesSubordinadas(authState); // Obtém o valor do prefixo relacionado ao usuário logado
  ...
  const obterSubordinada = () => {
    const prefixoDestinoUsuario = getJurisdicoesSubordinadas(authState); // Obtém o valor do prefixo relacionado ao usuário logado
    return getJurisdicoesSubordinadas(prefixoDestinoUsuario)
