...
function FormParamAlcadas({ location }) {
  ...
  function buscarPrefixosSubordinados(cod_dependencia) {
    if (cod_dependencia) {
      setPrefixoDestino(cod_dependencia);
      getJurisdicoesSubordinadas(cod_dependencia)
        .then((result) => {
          const jurisdicoes = result.map((item) => item.prefixo);
  ...
  useEffect(() => {
    if (prefixoDestino) {
      buscarPrefixosSubordinados(prefixoDestino);
      getJurisdicoesSubordinadas(prefixoDestino)
        .then((jurisdicoes) => {
          setPrefixosSubordinados(jurisdicoes);
          console.log(jurisdicoes)
