...
function FormParamAlcadas({ location }) {
  ...
  const [comissaoDestinoData, setComissaoDestinoData] = useState({ cod_cargo: '', nome_cargo: '' });
  ...
  function buscarComissaoDestino(cod_dependencia) {
    if (cod_dependencia) {
      getCargosComissoesFot09(cod_dependencia)
        .then((result) => {
          if (result.length > 0) {
            const { cod_cargo, nome_cargo } = result[0];
            setComissaoDestinoData({ cod_cargo, nome_cargo });
          } else {
            setComissaoDestinoData({ cod_cargo: '', nome_cargo: '' });
          }
        })
  ...
  return (
    <>
      ...
      <Card>
        <Form
          initialValues={{
            comissaoDestino: comissaoDestinoData.cod_cargo,
            nomeComissaoDestino: comissaoDestinoData.nome_cargo,
            comite,
            nomeComite,
            observacao,
          }}
        ...
        >
          ...
          <Form.Item name="comissaoDestino" label="Comissão Destino" required>
            <Input type="text" required placeholder="Código do Cargo" />
          </Form.Item>
          <Form.Item
