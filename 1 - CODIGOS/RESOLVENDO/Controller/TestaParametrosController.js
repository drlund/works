...
const UcVerificarStatusParametro = use("App/Commons/Movimentacoes/UseCases/UcVerificarStatusParametro");
const UcAtualizarStatusParametro = use("App/Commons/Movimentacoes/UseCases/UcAtualizarStatusParametro");
...
class TestaParametrosController {
  ...
  async gravarParametro({ request, response, session }) {
    ...
    if (parametroGravado) {
      const statusParametroExistente = await verificarStatusParametro(
        parametroExistente.prefixoDestino,
        parametroExistente.comissaoDestino
      );

      if (statusParametroExistente === "0") {
        await atualizarStatusParametro(
          parametroExistente.prefixoDestino,
          parametroExistente.comissaoDestino,
          "1"
        );
  ...
  async verificarStatusParametro({ response, request }) {
    const { prefixo } = request.allParams();
    const ucVerificarStatusParametro = new UcVerificarStatusParametro(new TestaParametrosRepository());
  ...
  async atualizarStatusParametro({ response, request }) {
    ...
    const ucAtualizarStatusParametro = new UcAtualizarStatusParametro(new TestaParametrosRepository());
    await ucAtualizarStatusParametro.validate(prefixoDestino, comissaoDestino, ativo);
